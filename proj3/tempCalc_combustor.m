...
while abs((target-right)/target) > 0.1 %this tolerance value has to be high(weak), and dT low for loop to converge
    T_guess = T_guess + dT;
    deltaH_CO2 = sp_heats_CO2(T_guess)/1000 * (44.01/1000) * dT; %the (x/1000) converts it to a per molar basis
    deltaH_H2O = sp_heats_H2O(T_guess)/1000 * (18.02/1000) * dT;
    deltaH_N2 = sp_heats_N2(T_guess)/1000 * (28.02/1000) * dT;
